...
export class InjectionRule implements Rule {
  ...
  async evaluate(context: ProjectContext): Promise<Finding[]> {
    ...
    const sinkPrefilter =
      /\.(?:query|execute|executeQuery|executemany|executescript|raw|rawQuery)\s*\(|\b(?:exec|execSync|execFile|spawn|system|popen|eval|Function)\s*\(|\bProcess\s*\.\s*(?:run|start)\s*\(|document\s*\.\s*write\s*\(|\.(?:innerHTML|outerHTML)\s*=|\b(?:res|response)\s*\.\s*(?:send|write|end)\s*\(|\breply\s*\.\s*send\s*\(|\brender_template_string\s*\(|\bsubprocess\s*\.\s*(?:run|call|check_output|Popen)\s*\(|\bos\s*\.\s*(?:system|popen)\s*\(|\bfetch\s*\(|\baxios\s*(?:\.\s*(?:get|post|put|delete|patch|head|options|request))?\s*\(|\bhttps?\s*\.\s*(?:get|request)\s*\(|\brequests\s*\.\s*(?:get|post|put|delete|patch|head|options|request)\s*\(|\b(?:urllib\s*\.\s*request\s*\.\s*)?urlopen\s*\(|\bfs(?:\s*\.\s*promises)?\s*\.\s*(?:readFile|readFileSync|createReadStream|writeFile|writeFileSync|createWriteStream|appendFile|appendFileSync|open|openSync|unlink|unlinkSync|stat|statSync|lstat|lstatSync|readdir|readdirSync)\s*\(|\bres\s*\.\s*(?:redirect|location|setHeader|header|cookie|writeHead|append)\s*\(|\bset(?:Timeout|Interval)\s*\(|\$(?:where|function|accumulator)\b|\b(?:c?pickle|_pickle|marshal|yaml)\s*\.\s*(?:loads?|load_all|unsafe_load|full_load)\s*\(|\bxpath\s*\.\s*(?:select1?|evaluate)\s*\(|\.(?:selectSingleNode|selectNodes|search_s|searchEntries?)\s*\(|\bldap(?:client)?\s*\.\s*search\s*\(|\bimport\s*\(/i;
    const sourcePrefilter =
      /\b(?:req|request)\s*\??\.\s*(?:body|query|params|args|form|values|json|cookies|headers|files|file|data|query_params|path_params|session|signedCookies|rawHeaders)\b|\b(?:req|request)\s*\[\s*['"](?:body|query|params|file|files|headers|cookies|args|form|session)['"]\s*\]|\bprocess\s*\.\s*(?:env|stdin)\b|\bstdin\b|\b(?:input|userInput|data|payload)\b|\b(?:URLSearchParams|location\s*\.\s*search|searchParams\s*\.\s*get|urlParams\s*\.\s*get|queryParams\s*\.\s*get|params\s*\.\s*get)\b|\(\s*\{[^}]*\b(?:query|body|params|headers|cookies|signedCookies|session|files|rawHeaders|queryParams|pathParams)\b/i;
  ...
  private _cweForSink(kind: string): string {
    switch (kind) {
      ...
      case 'header': return 'CWE-113';   // HTTP response splitting / header injection
